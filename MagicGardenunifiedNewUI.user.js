// ==UserScript==
// @name         MGTools Hybrid Unified (Complete)
// @namespace    http://tampermonkey.net/
// @version      3.0.0
// @description  Full feature parity of MagicGardenUnified inside the Hybrid dock+sidebar UI; preserved logic, revamped UI, fixed drag/sizing
// @author       MGTools Team
...
(function() {
    ...
    // Make globally accessible for integration
    targetWindow.MGA_CONTEXT = isUserscript ? 'userscript' : 'console';

    productionLog('🚀 MGTools Hybrid Complete initializing...');
    productionLog('📍 Context:', targetWindow.MGA_CONTEXT);
    ...
    GM_addStyle(`
        ...
        /* Tooltip positioning for horizontal dock */
        ...
        /* Tooltip positioning for vertical dock */
        ...
        .mgh-popout-body::-webkit-scrollbar { width: 6px; }
        .mgh-popout-body::-webkit-scrollbar-thumb {
            background: rgba(255, 255, 255, 0.2);
            border-radius: 3px;
        }

        .mgh-popout.collapsed .mgh-popout-body {
            display: none;
        }

        /* ==================== MINIMAL MODE STYLES ==================== */
        .mgh-minimal-container {
        ...
        .mgh-minimal-section-title:hover {
            background: #0a0a0a;
            color: #666;
        }

        .mgh-minimal-section.collapsed .mgh-minimal-content {
            display: none;
        }
        ...
        .mgh-dock-item:hover .mgh-popout-icon {
            opacity: 1;
        }

        /* Draggable dock handle */
        .mgh-dock-handle {
            width: 44px;
            height: 44px;
        ...
        .mgh-dock-handle:hover {
            background: rgba(255, 255, 255, 0.08);
        }

        /* Auto-hide tabs */
        .mgh-dock-item.auto-hide {
            opacity: 0.3;
            transform: scale(0.8);
        }

        .mgh-dock-item.auto-hide:hover {
            opacity: 1;
            transform: scale(1);
        }
    `);
    ...
    // ==================== DATA STRUCTURES ====================
    const TABS = [
        { id: 'minimal', icon: '▦', label: 'Minimal', autoHide: false },
        { id: 'overview', icon: '📊', label: 'Overview', autoHide: false },
        { id: 'pets', icon: '🐾', label: 'Pets', autoHide: false },
        { id: 'seeds', icon: '🌱', label: 'Seeds', autoHide: false },
        { id: 'values', icon: '💰', label: 'Values', autoHide: false },
        { id: 'rooms', icon: '🚪', label: 'Rooms', autoHide: false },
        { id: 'timers', icon: '⏰', label: 'Timers', autoHide: false },
        { id: 'abilities', icon: '⚡', label: 'Abilities', autoHide: false },
        { id: 'tools', icon: '🔧', label: 'Tools', autoHide: true },
        { id: 'hotkeys', icon: '⌨️', label: 'Hotkeys', autoHide: true },
        { id: 'alerts', icon: '🔔', label: 'Alerts', autoHide: true },
        { id: 'settings', icon: '⚙️', label: 'Settings', autoHide: true },
        { id: 'help', icon: '❓', label: 'Help', autoHide: false }
    ];
    ...
    const UnifiedState = {
        initialized: false,
        activeTab: null,
        dockOrientation: GM_getValue('mgh_dock_orientation', 'horizontal'), // 'horizontal' or 'vertical'
        popouts: GM_getValue('mgh_popouts', {}), // Track popout positions and states
        data: {
            petPresets: GM_getValue('MGA_petPresets', {}),
            petPresetsOrder: GM_getValue('MGA_petPresetsOrder', []),
            seedsToDelete: GM_getValue('MGA_seedsToDelete', []),
            petAbilityLogs: GM_getValue('MGA_petAbilityLogs', []),
            settings: GM_getValue('MGA_settings', {
                opacity: 95,
                theme: 'default',
                soundEnabled: true,
            ...
            autoDeleteEnabled: GM_getValue('MGA_autoDeleteEnabled', false),
            inventoryValue: 0,
            gardenValue: 0,
            tileValue: 0
        },
    ...
    // Make UnifiedState globally accessible
    targetWindow.UnifiedState = UnifiedState;

    // ==================== STORAGE FUNCTIONS ====================
    function MGA_saveJSON(key, data) {
        try {
            const jsonString = JSON.stringify(data);
            GM_setValue(key, data);

            // Verification
            const verification = GM_getValue(key);
            const verificationString = JSON.stringify(verification);

            if (verificationString !== jsonString) {
                productionError(`❌ [STORAGE] Save verification failed for ${key}`);
                return false;
            }

            productionLog(`✅ [STORAGE] Saved ${key}:`, Object.keys(data).length, 'items');
            return true;
        } catch (error) {
            productionError(`❌ [STORAGE] Failed to save ${key}:`, error);
            return false;
        }
    }

    function MGA_loadJSON(key, defaultValue = {}) {
        try {
            const data = GM_getValue(key, defaultValue);
            productionLog(`📦 [STORAGE] Loaded ${key}:`, typeof data === 'object' ? Object.keys(data).length : 0, 'items');
            return data;
        } catch (error) {
            productionError(`❌ [STORAGE] Failed to load ${key}:`, error);
            return defaultValue;
        }
    }
    ...
    // ==================== SPECIES VALUES ====================
    const speciesValues = {
        "Tomato": 1, "Carrot": 2, "Cabbage": 3, "Turnip": 4, "Radish": 5,
        "Corn": 6, "Potato": 7, "Wheat": 8, "Rice": 9, "Lettuce": 10,
        "Spinach": 11, "Kale": 12, "Broccoli": 13, "Cauliflower": 14,
        "Cucumber": 15, "Zucchini": 16, "Pumpkin": 17, "Watermelon": 18,
        "Cantaloupe": 19, "Honeydew": 20, "Strawberry": 25, "Blueberry": 30,
        "Raspberry": 35, "Blackberry": 40, "Cherry": 50, "Peach": 60,
        "Plum": 70, "Apple": 80, "Pear": 90, "Grape": 100, "Lemon": 120,
        "Lime": 140, "Orange": 160, "Tangerine": 180, "Grapefruit": 200,
        "Mango": 250, "Papaya": 300, "Pineapple": 350, "Banana": 400,
        "Coconut": 500, "Avocado": 600
    };
    ...
    function calculateMutationMultiplier(mutations) {
        ...
        const COLOR_MULT = {
            Gray: 1, White: 1.2, Yellow: 1.44, Orange: 1.73, Pink: 2.07,
            Red: 2.49, Purple: 2.99, Blue: 3.58, Green: 4.3, Black: 5.16,
            Rainbow: 6.19
        };

        const WEATHER_MULT = {
            Clear: 1, Cloudy: 1.2, Rainy: 1.44, Snowy: 1.73, Foggy: 2.07,
            Stormy: 2.49, Windy: 2.99, Tornado: 3.58, Hurricane: 4.3,
            Sunny: 5.16, Eclipse: 6.19
        };

        const TIME_MULT = {
            Day: 1, Dusk: 1.2, Night: 1.44, Dawn: 1.73, Noon: 2.07,
            Midnight: 2.49, Twilight: 2.99, Sunrise: 3.58, Sunset: 4.3
        };

        const WEATHER_TIME_COMBO = {
            "Sunny+Noon": 10.33, "Eclipse+Midnight": 15.5
        };

        let color = 1, weather = null, time = null;

        mutations.forEach(m => {
            if (COLOR_MULT[m]) color = COLOR_MULT[m];
            else if (WEATHER_MULT[m]) weather = m;
            else if (TIME_MULT[m]) time = m;
        });
    ...
    function calculateTileValue() {
        ...
        currentCrop.forEach(slot => {
            if (slot && slot.species) {
                ...
                total += Math.round(multiplier * speciesVal * scale * friendBonus);
            }
        });
    ...
    function calculateGardenValue() {
        const myGarden = UnifiedState.atoms.myGarden;
        const friendBonus = UnifiedState.atoms.friendBonus || 1;
        let total = 0;

        if (myGarden && myGarden.tileObjects) {
            const tiles = Array.isArray(myGarden.tileObjects) ?
                myGarden.tileObjects :
                (myGarden.tileObjects instanceof Map ?
                    Array.from(myGarden.tileObjects.values()) :
                    Object.values(myGarden.tileObjects));

            tiles.forEach(tile => {
                if (tile && tile.species && tile.itemType === 'Produce') {
                    const multiplier = calculateMutationMultiplier(tile.mutations);
                    const speciesVal = speciesValues[tile.species] || 0;
    ...
    function getContent(id) {
        switch(id) {
            case 'minimal':
                return getMinimalContent();
            case 'overview':
                return getOverviewContent();
            case 'pets':
                return getPetsContent();
            case 'seeds':
                return getSeedsContent();
            case 'values':
                return getValuesContent();
            case 'rooms':
                return getRoomsContent();
            case 'timers':
                return getTimersContent();
            case 'abilities':
                return getAbilitiesContent();
            case 'tools':
                return getToolsContent();
            case 'hotkeys':
                return getHotkeysContent();
            case 'alerts':
            ...
            default:
                return `<div style="padding: 40px; text-align: center; opacity: 0.5;">${id} coming soon</div>`;
        }
    }

    function getMinimalContent() {
        const gold = UnifiedState.atoms.gold || 0;
        const essence = UnifiedState.atoms.essence || 0;
        const tileValue = calculateTileValue();
        const activePets = UnifiedState.atoms.activePets || [];
        const presets = Object.keys(UnifiedState.data.petPresets);
        const seedsToDelete = UnifiedState.data.seedsToDelete || [];

        return `
            <div class="mgh-minimal-container">
                <div class="mgh-minimal-stats">
                    <div class="mgh-minimal-stat">
                        <div class="mgh-minimal-stat-label">GOLD</div>
                        <div class="mgh-minimal-stat-value">${(gold / 1000).toFixed(1)}K</div>
                    </div>
                    <div class="mgh-minimal-stat">
                        <div class="mgh-minimal-stat-label">ESSENCE</div>
                        <div class="mgh-minimal-stat-value">${(essence / 1000000).toFixed(1)}M</div>
                    </div>
                    <div class="mgh-minimal-stat">
                        <div class="mgh-minimal-stat-label">TILE</div>
                        <div class="mgh-minimal-stat-value">${(tileValue / 1000).toFixed(1)}K</div>
                    </div>
                </div>

                <div class="mgh-minimal-section">
                    <div class="mgh-minimal-section-title" data-section="pets">
                        PETS <span>▼</span>
                    </div>
                    <div class="mgh-minimal-content">
                        ${activePets.length > 0 ? activePets.map(p => `
                            <div class="mgh-minimal-row">
                                <div class="mgh-minimal-label">${p.petSpecies || 'Unknown'}</div>
                                <div class="mgh-minimal-value">${p.hunger || 0}%</div>
                            </div>
                        `).join('') : '<div class="mgh-minimal-row"><div class="mgh-minimal-label">No pets active</div></div>'}
                    </div>
                </div>

                <div class="mgh-minimal-section">
                    <div class="mgh-minimal-section-title" data-section="presets">
                        PRESETS <span>▼</span>
                    </div>
                    <div class="mgh-minimal-content">
                        ${presets.length > 0 ? presets.slice(0, 5).map(name => `
                            <div class="mgh-minimal-row minimal-preset-load" data-name="${name}">
                                <div class="mgh-minimal-label">${name}</div>
                                <div class="mgh-minimal-value">→</div>
                            </div>
                        `).join('') : '<div class="mgh-minimal-row"><div class="mgh-minimal-label">No presets</div></div>'}
                    </div>
                </div>

                <div class="mgh-minimal-section">
                    <div class="mgh-minimal-section-title" data-section="seeds">
                        SEEDS <span>▼</span>
                    </div>
                    <div class="mgh-minimal-content">
                        <div class="mgh-minimal-row">
                            <div class="mgh-minimal-label">Auto-Delete</div>
                            <div class="mgh-minimal-value">${seedsToDelete.length}</div>
                        </div>
                    </div>
                </div>

                <div class="mgh-minimal-section">
                    <div class="mgh-minimal-section-title" data-section="rooms">
                        ROOMS <span>▼</span>
                    </div>
                    <div class="mgh-minimal-content">
                        ${Object.keys(UnifiedState.data.roomStatus.counts).length > 0 ?
                            Object.keys(UnifiedState.data.roomStatus.counts).map(room => `
                                <div class="mgh-minimal-row">
                                    <div class="mgh-minimal-label">${room}</div>
                                    <div class="mgh-minimal-value">${UnifiedState.data.roomStatus.counts[room]}</div>
                                </div>
                            `).join('') :
                            '<div class="mgh-minimal-row"><div class="mgh-minimal-label">No room data</div></div>'
                        }
                    </div>
                </div>
            </div>
    ...
    function getOverviewContent() {
        ...
        const essence = UnifiedState.atoms.essence || 0;
        const activePets = UnifiedState.atoms.activePets || [];
        const presetCount = Object.keys(UnifiedState.data.petPresets).length;

        return `
            <div class="mgh-section">
                <div class="mgh-section-title">Quick Stats</div>
                <div class="mgh-value-grid">
                    <div class="mgh-value-label">Gold:</div>
                    <div class="mgh-value-amount">${gold.toLocaleString()}</div>
                    <div class="mgh-value-label">Essence:</div>
                    <div class="mgh-value-amount">${essence.toLocaleString()}</div>
                    <div class="mgh-value-label">Active Pets:</div>
                    <div class="mgh-value-amount">${activePets.length}</div>
                    <div class="mgh-value-label">Pet Presets:</div>
                    <div class="mgh-value-amount">${presetCount}</div>
                </div>
            </div>
            <div class="mgh-section">
                <div class="mgh-section-title">Quick Actions</div>
                <button class="mgh-btn" style="width: 100%; margin-bottom: 8px;" onclick="UnifiedState.activeTab = 'pets'; updateSidebar()">Manage Pets</button>
                <button class="mgh-btn" style="width: 100%; margin-bottom: 8px;" onclick="UnifiedState.activeTab = 'seeds'; updateSidebar()">Delete Seeds</button>
                <button class="mgh-btn" style="width: 100%;" onclick="UnifiedState.activeTab = 'values'; updateSidebar()">View Values</button>
            </div>
        `;
    }

    function getPetsContent() {
        const activePets = UnifiedState.atoms.activePets || targetWindow.activePets || [];
        const petPresets = UnifiedState.data.petPresets;
        const order = UnifiedState.data.petPresetsOrder;

        let html = `<div class="mgh-section">`;

        // Active Pets Display
        html += `<div class="mgh-section-title">Active Pets</div>`;
        if (activePets.length > 0) {
            activePets.forEach(pet => {
                html += `<span class="mgh-pet-badge">${pet.petSpecies || 'Unknown'}</span>`;
            });
        } else {
            html += `<div style="padding: 16px; text-align: center; opacity: 0.5;">No active pets</div>`;
        }
        html += `</div>`;

        // Quick Load Dropdown
        html += `<div class="mgh-section">`;
        html += `<div class="mgh-section-title">Quick Load Preset</div>`;
        html += `<select id="p-sel" class="mgh-select">`;
        ...
        // Save New Preset
        html += `<div class="mgh-section">`;
        html += `<div class="mgh-section-title">Save Current Pets</div>`;
        html += `<input type="text" id="preset-name" class="mgh-input" placeholder="Preset name...">`;
        html += `<button class="mgh-btn" id="save-preset-btn" style="width: 100%;">Save Preset</button>`;
        html += `</div>`;

        // Saved Presets
        html += `<div class="mgh-section">`;
        html += `<div class="mgh-section-title">Saved Presets</div>`;
        if (order.length === 0) {
            html += `<div style="padding: 16px; text-align: center; opacity: 0.5;">No presets saved</div>`;
        } else {
            order.forEach(name => {
                const preset = petPresets[name];
                const species = preset.map(p => p.petSpecies).join(', ');
                html += `
                    <div class="mgh-preset">
                        <div class="mgh-preset-name">${name}</div>
                        <div class="mgh-preset-species">${species}</div>
                        <div style="display: flex; gap: 8px;">
                            <button class="mgh-btn p-load" data-n="${name}" style="flex: 1;">Load</button>
                            <button class="mgh-btn mgh-btn-danger p-delete" data-n="${name}" style="flex: 1;">Delete</button>
                        </div>
                    </div>
                `;
            });
    ...
    function getSeedsContent() {
        const seedsToDelete = UnifiedState.data.seedsToDelete || [];
        const allSeeds = Object.keys(speciesValues);

        let html = `<div class="mgh-section">`;
        html += `<div class="mgh-section-title">Select Seeds to Auto-Delete</div>`;

        allSeeds.forEach(seed => {
            const checked = seedsToDelete.includes(seed) ? 'checked' : '';
            html += `
                <div class="mgh-checkbox-group">
                    <input type="checkbox" class="mgh-checkbox seed-checkbox" data-seed="${seed}" ${checked}>
                    <label class="mgh-checkbox-label">${seed}</label>
                </div>
            `;
        });
        ...
        html += `</div>`;
        html += `<div class="mgh-section">`;
        html += `<button class="mgh-btn" id="save-seeds-btn" style="width: 100%; margin-bottom: 8px;">Save Selection</button>`;
        html += `<button class="mgh-btn mgh-btn-danger" id="clear-seeds-btn" style="width: 100%;">Clear All</button>`;
        html += `</div>`;
    ...
    function getValuesContent() {
        ...
        const gardenValue = calculateGardenValue();
        ...
        return `
            <div class="mgh-section">
                <div class="mgh-section-title">Garden Values</div>
                <div class="mgh-value-grid">
                    <div class="mgh-value-label">Tile Value:</div>
                    <div class="mgh-value-amount">${tileValue.toLocaleString()}</div>
                    <div class="mgh-value-label">Inventory Value:</div>
                    <div class="mgh-value-amount">${inventoryValue.toLocaleString()}</div>
                    <div class="mgh-value-label">Garden Value:</div>
                    <div class="mgh-value-amount">${gardenValue.toLocaleString()}</div>
                    <div class="mgh-value-label">Total Value:</div>
                    <div class="mgh-value-amount">${(tileValue + inventoryValue + gardenValue).toLocaleString()}</div>
                </div>
            </div>
        `;
    }
    ...
    function getRoomsContent() {
        const roomStatus = UnifiedState.data.roomStatus;
        const counts = roomStatus.counts || {};

        let html = `<div class="mgh-section">`;
        html += `<div class="mgh-section-title">Room Player Counts</div>`;

        if (Object.keys(counts).length === 0) {
            html += `<div style="padding: 16px; text-align: center; opacity: 0.5;">No room data available</div>`;
        } else {
            Object.keys(counts).sort().forEach(room => {
                html += `
                    <div style="display: flex; justify-content: space-between; padding: 8px; background: rgba(255,255,255,0.05); border-radius: 6px; margin-bottom: 8px;">
                        <span>${room}</span>
                        <span style="color: #4a9eff; font-weight: 600;">${counts[room]} players</span>
                    </div>
                `;
            });
        }
    ...
    function getTimersContent() {
        return `
            <div class="mgh-section">
                <div class="mgh-section-title">Restock Timers</div>
                <div style="display: grid; gap: 12px;">
                    <div style="display: flex; justify-content: space-between; padding: 12px; background: rgba(255,255,255,0.05); border-radius: 8px;">
                        <span>Seed Restock:</span>
                        <span id="timer-seed" style="color: #4a9eff; font-weight: 600;">--:--</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; padding: 12px; background: rgba(255,255,255,0.05); border-radius: 8px;">
                        <span>Egg Restock:</span>
                        <span id="timer-egg" style="color: #4a9eff; font-weight: 600;">--:--</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; padding: 12px; background: rgba(255,255,255,0.05); border-radius: 8px;">
                        <span>Tool Restock:</span>
                        <span id="timer-tool" style="color: #4a9eff; font-weight: 600;">--:--</span>
                    </div>
                </div>
            </div>
        `;
    ...
    function getAbilitiesContent() {
        const logs = UnifiedState.data.petAbilityLogs || [];

        let html = `<div class="mgh-section">`;
        html += `<div class="mgh-section-title">Pet Ability Logs</div>`;

        if (logs.length === 0) {
            html += `<div style="padding: 16px; text-align: center; opacity: 0.5;">No ability logs yet</div>`;
        } else {
            logs.slice(-10).reverse().forEach(log => {
                html += `
                    <div style="padding: 8px; background: rgba(255,255,255,0.05); border-radius: 6px; margin-bottom: 8px; font-size: 12px;">
                        <div style="color: #4a9eff; font-weight: 600;">${log.petSpecies} - ${log.ability}</div>
                        <div style="color: rgba(255,255,255,0.6); font-size: 11px;">${log.timestamp}</div>
                    </div>
                `;
            });
        }
        ...
        html += `</div>`;
        html += `<button class="mgh-btn mgh-btn-danger" id="clear-logs-btn" style="width: 100%;">Clear Logs</button>`;
        return html;
    }
    ...
    function getToolsContent() {
        return `
            <div class="mgh-section">
                <div class="mgh-section-title">Crop Tools</div>
                <div style="padding: 16px; text-align: center; opacity: 0.5;">Coming soon: Crop highlighting and utilities</div>
            </div>
        `;
    }

    function getHotkeysContent() {
        return `
            <div class="mgh-section">
                <div class="mgh-section-title">Keyboard Shortcuts</div>
                <div style="padding: 16px; text-align: center; opacity: 0.5;">Coming soon: Customizable hotkeys</div>
            </div>
        `;
    }
    ...
    function getAlertsContent() {
        return `
            <div class="mgh-section">
                <div class="mgh-section-title">Notifications</div>
                <div style="padding: 16px; text-align: center; opacity: 0.5;">Coming soon: Alert configuration</div>
            </div>
        `;
    }
    ...
    function getSettingsContent() {
        ...
        return `
            <div class="mgh-section">
                <div class="mgh-section-title">Appearance</div>
                <div style="margin-bottom: 12px;">
                    <label style="display: block; margin-bottom: 4px; font-size: 12px; color: rgba(255,255,255,0.7);">
                        Opacity: ${settings.opacity}%
                    </label>
                    <input type="range" id="opacity-slider" min="0" max="100" value="${settings.opacity}"
                           style="width: 100%;">
                </div>
                <div style="margin-bottom: 12px;">
                    <label style="display: block; margin-bottom: 4px; font-size: 12px; color: rgba(255,255,255,0.7);">
                    ...
                    <select id="theme-select" class="mgh-select">
            ...
            <div class="mgh-section">
                <div class="mgh-section-title">Data Management</div>
                <button class="mgh-btn" id="export-data-btn" style="width: 100%; margin-bottom: 8px;">Export All Data</button>
                <button class="mgh-btn" id="import-data-btn" style="width: 100%; margin-bottom: 8px;">Import Data</button>
                <button class="mgh-btn mgh-btn-danger" id="reset-data-btn" style="width: 100%;">Reset All Data</button>
            </div>
        `;
    }
    ...
    function getHelpContent() {
        return `
            <div class="mgh-section">
                <div class="mgh-section-title">MGTools Hybrid</div>
                <div style="font-size: 12px; line-height: 1.6; color: rgba(255,255,255,0.7);">
                    <p>Version 2.0.0 - Complete edition with hybrid dock+sidebar design</p>
                    <p style="margin-top: 12px;"><strong>Features:</strong></p>
                    <ul style="margin: 8px 0; padding-left: 20px;">
                        <li>Pet preset management</li>
                        <li>Seed auto-deletion</li>
                        <li>Value calculations</li>
                        <li>Room status tracking</li>
                        <li>Restock timers</li>
                        <li>Ability logging</li>
                        <li>And more...</li>
                    </ul>
                    <p style="margin-top: 12px;"><strong>Tips:</strong></p>
                    <ul style="margin: 8px 0; padding-left: 20px;">
                        <li>Click dock icons to open sidebar</li>
                        <li>Use flip button (⇄) to switch dock orientation</li>
                        <li>Click outside sidebar to close</li>
                        <li>Press ESC to close sidebar</li>
                    </ul>
                </div>
            </div>
        `;
    ...
    // ==================== EVENT HANDLERS ====================
    function setupHandlers(id) {
        if (id === 'pets') {
            // Quick load dropdown
            const select = document.getElementById('p-sel');
            if (select) {
                select.addEventListener('change', (e) => {
                    if (e.target.value && typeof targetWindow.loadPreset === 'function') {
                        targetWindow.loadPreset(e.target.value);
                    }
                });
            }
            ...
            if (saveBtn) {
                saveBtn.addEventListener('click', () => {
                    ...
                    const activePets = UnifiedState.atoms.activePets || targetWindow.activePets || [];
                    if (activePets.length === 0) {
                        alert('No active pets to save');
                        return;
                    }

                    UnifiedState.data.petPresets[name] = activePets;
                    ...
                    input.value = '';
                    openSidebar(TABS.find(t => t.id === 'pets'));
                    productionLog('✅ Preset saved:', name);
                });
            }

            // Load buttons
            document.querySelectorAll('.p-load').forEach(btn => {
                btn.addEventListener('click', () => {
                    const name = btn.dataset.n;
                    if (typeof targetWindow.loadPreset === 'function') {
                        targetWindow.loadPreset(name);
                    }
                });
            });
            ...
            document.querySelectorAll('.p-delete').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    if (confirm(`Delete preset "${name}"?`)) {
                        delete UnifiedState.data.petPresets[name];
                        UnifiedState.data.petPresetsOrder = UnifiedState.data.petPresetsOrder.filter(n => n !== name);

                        MGA_saveJSON('MGA_petPresets', UnifiedState.data.petPresets);
                        MGA_saveJSON('MGA_petPresetsOrder', UnifiedState.data.petPresetsOrder);

                        openSidebar(TABS.find(t => t.id === 'pets'));
                        productionLog('🗑️ Preset deleted:', name);
                    }
                });
            });
        }
            ...
            const clearBtn = document.getElementById('clear-seeds-btn');

            if (saveBtn) {
                saveBtn.addEventListener('click', () => {
                    const checkboxes = document.querySelectorAll('.seed-checkbox:checked');
                    UnifiedState.data.seedsToDelete = Array.from(checkboxes).map(cb => cb.dataset.seed);
                    MGA_saveJSON('MGA_seedsToDelete', UnifiedState.data.seedsToDelete);
                    productionLog('✅ Seeds to delete saved:', UnifiedState.data.seedsToDelete.length);
                });
            }
            ...
            if (clearBtn) {
                clearBtn.addEventListener('click', () => {
                    ...
                    MGA_saveJSON('MGA_seedsToDelete', []);
                    productionLog('🗑️ Seed selection cleared');
                });
            }
        }
            ...
            if (clearBtn) {
                clearBtn.addEventListener('click', () => {
                    if (confirm('Clear all ability logs?')) {
                        ...
                        openSidebar(TABS.find(t => t.id === 'abilities'));
                        productionLog('🗑️ Ability logs cleared');
                    }
                });
            }
        }
            ...
            if (opacitySlider) {
                opacitySlider.addEventListener('input', (e) => {
                    UnifiedState.data.settings.opacity = parseInt(e.target.value);
                    MGA_saveJSON('MGA_settings', UnifiedState.data.settings);
                    const sidebar = document.getElementById('mgh-sidebar');
                    if (sidebar) {
                        sidebar.style.opacity = UnifiedState.data.settings.opacity / 100;
                    }
                });
            }
            ...
            if (themeSelect) {
                themeSelect.addEventListener('change', (e) => {
                    UnifiedState.data.settings.theme = e.target.value;
                    MGA_saveJSON('MGA_settings', UnifiedState.data.settings);
                    productionLog('🎨 Theme changed to:', e.target.value);
                });
            }
            ...
            if (exportBtn) {
                exportBtn.addEventListener('click', () => {
                    const data = {
                        ...
                        petPresetsOrder: UnifiedState.data.petPresetsOrder,
                    ...
                    a.href = url;
                    a.download = 'mgtools-data.json';
                    a.click();
                    productionLog('📤 Data exported');
                });
            }
            ...
            if (resetBtn) {
                resetBtn.addEventListener('click', () => {
                    if (confirm('Reset ALL data? This cannot be undone!')) {
                        ...
                        MGA_saveJSON('MGA_petPresetsOrder', []);
                        MGA_saveJSON('MGA_seedsToDelete', []);

                        openSidebar(TABS.find(t => t.id === 'settings'));
                        productionLog('🔄 All data reset');
                    }
                });
            }
        }
    ...
    function createDock() {
        ...
        dock.id = 'mgh-dock';
        dock.className = UnifiedState.dockOrientation;

        // Add flip toggle button
        const flipBtn = document.createElement('div');
        flipBtn.className = 'mgh-dock-item flip-toggle';
        flipBtn.innerHTML = `⇄<div class="mgh-tooltip">Flip Orientation</div>`;
        flipBtn.addEventListener('click', () => {
            UnifiedState.dockOrientation = UnifiedState.dockOrientation === 'horizontal' ? 'vertical' : 'horizontal';
            GM_setValue('mgh_dock_orientation', UnifiedState.dockOrientation);
            dock.className = UnifiedState.dockOrientation;
            productionLog('🔄 Dock orientation:', UnifiedState.dockOrientation);
        });
        dock.appendChild(flipBtn);

        // Add tab buttons
        TABS.forEach(item => {
            const itemDiv = document.createElement('div');
            itemDiv.className = 'mgh-dock-item';
            itemDiv.dataset.id = item.id;
            itemDiv.innerHTML = `${item.icon}<div class="mgh-tooltip">${item.label}</div>`;
    ...
    function createSidebar() {
        ...
        sidebar.innerHTML = `
            <div class="mgh-sidebar-header">
                <div class="mgh-sidebar-title" id="mgh-sidebar-title"></div>
                <button class="mgh-sidebar-close">×</button>
            </div>
            <div class="mgh-sidebar-body" id="mgh-sidebar-body"></div>
        `;

        sidebar.querySelector('.mgh-sidebar-close').addEventListener('click', closeSidebar);
        return sidebar;
    }

    function openSidebar(item) {
        const sidebar = document.getElementById('mgh-sidebar');
        const title = document.getElementById('mgh-sidebar-title');
        const body = document.getElementById('mgh-sidebar-body');

        activeSidebar = item.id;
        UnifiedState.activeTab = item.id;
        title.textContent = item.label;
        body.innerHTML = getContent(item.id);
        setupHandlers(item.id);

        sidebar.classList.add('open');

        document.querySelectorAll('.mgh-dock-item').forEach(i => i.classList.remove('active'));
        document.querySelector(`[data-id="${item.id}"]`)?.classList.add('active');

        productionLog('📂 Opened:', item.label);
    }

    function closeSidebar() {
        document.getElementById('mgh-sidebar').classList.remove('open');
        document.querySelectorAll('.mgh-dock-item').forEach(i => i.classList.remove('active'));
        activeSidebar = null;
        productionLog('📁 Sidebar closed');
    }

    function updateSidebar() {
        if (activeSidebar) {
            const item = TABS.find(t => t.id === UnifiedState.activeTab);
            if (item) {
                openSidebar(item);
            }
        }
    ...
    // Make updateSidebar globally accessible
    targetWindow.updateSidebar = updateSidebar;

    // ==================== TURTLE TIMER + TILE VALUE OVERLAY ====================
    function createTurtleOverlay() {
        const overlay = document.createElement('div');
        overlay.id = 'mgh-turtle-overlay';
        return overlay;
    }

    function updateTurtleOverlay() {
        const overlay = document.getElementById('mgh-turtle-overlay');
        if (!overlay) return;

        // Find the game's time display element
        ...
        if (timeElement) {
            ...
            // Turtle timer estimate
            const turtleEstimate = estimateUntilLatestCrop(
                UnifiedState.atoms.currentCrop || targetWindow.currentCrop,
                UnifiedState.atoms.activePets || targetWindow.activePets
            );

            if (turtleEstimate) {
                html += `<div class="mgh-timer-display">${turtleEstimate}</div>`;
            }

            // Tile value
            const tileValue = calculateTileValue();
            if (tileValue > 0) {
                html += `<div class="mgh-tile-value">${tileValue.toLocaleString()}</div>`;
            }
    ...
    // ==================== ATOM HOOKS ====================
    function hookAtom(filePath, globalKey, callback) {
        try {
            const originalConsoleLog = console.log;

            console.log = function(...args) {
                if (args[0] && typeof args[0] === 'string' && args[0].includes(filePath)) {
                    const value = args[1];
                    if (value !== undefined) {
                        targetWindow[globalKey] = value;
                        if (callback) {
                            const result = callback(value);
                            if (result !== undefined) {
                                targetWindow[globalKey] = result;
                            }
                        }
                    }
                }
            ...
            productionLog(`🔗 [HOOK] Hooked ${globalKey} from ${filePath.split('/').pop()}`);
        } catch (error) {
            productionError(`❌ [HOOK] Failed to hook ${globalKey}:`, error);
        }
    }

    function initializeAtomHooks() {
        // Hook active pets
        hookAtom(
            "/home/runner/work/magiccircle.gg/magiccircle.gg/client/src/games/Quinoa/atoms/_archive/myPetSlotsAtom.ts/myPetSlotsAtom",
            "activePets",
            (value) => {
                UnifiedState.atoms.activePets = value || [];
                updateTurtleOverlay();
                return value;
            }
        );
        ...
        hookAtom(
            ...
            "currentCrop",
            (value) => {
                UnifiedState.atoms.currentCrop = value || [];
                updateTurtleOverlay();
                return value;
            }
        );
        ...
        hookAtom(
            ...
            "inventory",
            (value) => {
                UnifiedState.atoms.inventory = value;
                return value;
            }
        );
        ...
        // Hook garden
        hookAtom(
            "/home/runner/work/magiccircle.gg/magiccircle.gg/client/src/games/Quinoa/atoms/myAtoms.ts/myGardenAtom",
            "myGarden",
            (value) => {
                UnifiedState.atoms.myGarden = value;
                return value;
            }
        );
        ...
        // Hook friend bonus
        hookAtom(
            "/home/runner/work/magiccircle.gg/magiccircle.gg/client/src/games/Quinoa/atoms/myAtoms.ts/myFriendBonusAtom",
            "friendBonus",
            (value) => {
        ...
        // Hook gold
        hookAtom(
            "/home/runner/work/magiccircle.gg/magiccircle.gg/client/src/games/Quinoa/atoms/myAtoms.ts/myGoldAtom",
            "gold",
            (value) => {
                UnifiedState.atoms.gold = value || 0;
                return value;
            }
        );
        ...
        // Hook essence
        hookAtom(
            "/home/runner/work/magiccircle.gg/magiccircle.gg/client/src/games/Quinoa/atoms/myAtoms.ts/myEssenceAtom",
            "essence",
            (value) => {
                UnifiedState.atoms.essence = value || 0;
                return value;
            }
        );

        productionLog('✅ Atom hooks initialized');
    }

    // ==================== INITIALIZATION ====================
    function initialize() {
        if (UnifiedState.initialized) {
            productionWarn('⚠️ Already initialized');
            return;
        }

        productionLog('🔧 Initializing MGTools Hybrid Complete...');

        // Create UI
        const dock = createDock();
        const sidebar = createSidebar();
        const turtleOverlay = createTurtleOverlay();
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('#mgh-sidebar') && !e.target.closest('#mgh-dock')) {
                closeSidebar();
            }
        });
        ...
        // Update turtle overlay periodically
        setInterval(updateTurtleOverlay, 1000);

        // Set initialized flag
        UnifiedState.initialized = true;

        productionLog('✅ MGTools Hybrid Complete loaded - Click dock icons!');
    }
...
(function() {
    ...
    const UNIFIED_STYLES = `
        ...
        .mga-tab-group-header.collapsed .mga-tab-group-icon {
        ...
        .mga-tab-group-header.collapsed + .mga-tab-group-tabs {
            max-height: 0;
            opacity: 0;
        }
        ...
        .mga-tab-vertical:hover {
            background: rgba(255, 255, 255, 0.2);
        }

        .mga-tab-vertical.active {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
        }

        .mga-tab-favorite-btn {
            opacity: 0.5;
            transition: opacity 0.2s ease;
            cursor: pointer;
            font-size: 14px;
        ...
        .mga-tab-favorite-btn:hover,
        .mga-tab-favorite-btn.favorited {
            opacity: 1;
            color: #fbbf24;
        }
        ...
        .mga-tab-search::placeholder {
            color: rgba(255, 255, 255, 0.5);
        }

        .mga-tab-search:focus {
            outline: none;
        ...
        .mga-favorite-tab:hover {
            background: rgba(255, 255, 255, 0.25);
        }

        .mga-favorite-tab.active {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
        }
...
function applyResponsiveTextScaling(overlay, width, height) {
    ...
    const UnifiedState = {
        ...
        activeTab: 'overview',  // Will be set based on UI mode during initialization
        ...
        data: {
            ...
            uiNavigation: {
                ...
                favorites: [],  // Array of favorited tab names
                searchQuery: '',
                legacyMode: false  // Toggle between old and new UI
            }
        },
        atoms: {
...
window.MGA_debugStorage = function() {
    ...
    const TAB_CONFIG = {
        overview: {
            group: null,  // Special - not in a group
            name: 'Overview',
            icon: '📊',
            tooltip: 'Dashboard overview of all features'
        },
        gameplay: {
            pets: {
                name: 'Pets',
                icon: '🐾',
                tooltip: 'Manage pet loadouts and analyze optimal combinations'
            },
            seeds: {
                name: 'Seeds',
                icon: 'S',
                tooltip: 'Mass seed deletion and inventory management'
            },
            tools: {
                name: 'Tools',
                icon: '🔧',
                tooltip: 'Crop highlighting and utilities'
            },
            rooms: {
                name: 'Rooms',
                icon: '🎮',
                tooltip: 'Live room player counts and quick join'
            }
        },
        analytics: {
            values: {
                name: 'Values',
                icon: '💰',
                tooltip: 'Resource dashboard with analytics and tracking'
            },
            timers: {
                name: 'Timers',
                icon: '⏰',
                tooltip: 'Event timers and restock countdowns'
            },
            abilities: {
                name: 'Abilities',
                icon: '⚡',
                tooltip: 'Track pet abilities and performance logs'
            }
        },
        config: {
            hotkeys: {
            ...
            notifications: {
                name: 'Alerts',
                icon: '🔔',
                tooltip: 'Configure alerts and sounds'
            },
            settings: {
                name: 'Settings',
                icon: '⚙️',
                tooltip: 'Appearance and general settings'
            },
            help: {
                name: 'Help',
                icon: '❓',
                tooltip: 'Keyboard shortcuts and user guide'
            }
        }
    };
    ...
    // Setup tab interactions (works for both legacy and new UI)
    function setupTabInteractions(tabs) {
        const { legacyMode } = UnifiedState.data.uiNavigation;
        ...
        if (legacyMode) {
            ...
            // New grouped vertical tabs

            // Tab search
            const searchInput = tabs.querySelector('.mga-tab-search');
            if (searchInput) {
                searchInput.addEventListener('input', (e) => {
                    const query = e.target.value.toLowerCase();
                    UnifiedState.data.uiNavigation.searchQuery = query;

                    // Filter tabs
                    tabs.querySelectorAll('.mga-tab-vertical').forEach(tab => {
                    ...
                    // Show/hide groups based on if they have visible tabs
                    tabs.querySelectorAll('.mga-tab-group').forEach(group => {
                        const visibleTabs = Array.from(group.querySelectorAll('.mga-tab-vertical')).some(t => t.style.display !== 'none');
                        group.style.display = visibleTabs ? 'flex' : 'none';
                    });
                });
            }

            // Favorite tabs
            tabs.querySelectorAll('.mga-favorite-tab').forEach(favTab => {
                favTab.addEventListener('click', () => {
                    const tabKey = favTab.dataset.tab;
                    tabs.querySelectorAll('.mga-tab-vertical, .mga-favorite-tab').forEach(t => t.classList.remove('active'));
                    favTab.classList.add('active');
                    const matchingTab = tabs.querySelector(`.mga-tab-vertical[data-tab="${tabKey}"]`);
                    if (matchingTab) matchingTab.classList.add('active');
                    UnifiedState.activeTab = tabKey;
                    updateTabContent();
                });
            });

            // Group headers (collapse/expand)
            tabs.querySelectorAll('.mga-tab-group-header').forEach(header => {
                header.addEventListener('click', () => {
                    const groupKey = header.dataset.group;
                    const isCollapsed = header.classList.contains('collapsed');
                    header.classList.toggle('collapsed');
                    UnifiedState.data.uiNavigation.groupsExpanded[groupKey] = isCollapsed;
                    MGA_saveJSON('MGA_uiNavigation', UnifiedState.data.uiNavigation);
                });
            });
            ...
            tabs.querySelectorAll('.mga-tab-vertical').forEach(tab => {
                tab.addEventListener('click', (e) => {
                    // Don't switch if clicking on favorite or popout button
                    if (e.target.classList.contains('mga-tab-favorite-btn') ||
                        e.target.classList.contains('mga-tab-popout')) {
                        e.stopPropagation();
                        return;
                    }
                    tabs.querySelectorAll('.mga-tab-vertical, .mga-favorite-tab').forEach(t => t.classList.remove('active'));
                    ...
                    // Update favorite bar if exists
                    const favTab = tabs.querySelector(`.mga-favorite-tab[data-tab="${tabKey}"]`);
            ...
            // Favorite buttons
            tabs.querySelectorAll('.mga-tab-favorite-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    const tabKey = btn.dataset.favorite;
                    const isFavorited = UnifiedState.data.uiNavigation.favorites.includes(tabKey);

                    if (isFavorited) {
                        UnifiedState.data.uiNavigation.favorites = UnifiedState.data.uiNavigation.favorites.filter(f => f !== tabKey);
                        btn.classList.remove('favorited');
                    } else {
                        UnifiedState.data.uiNavigation.favorites.push(tabKey);
                        btn.classList.add('favorited');
                    }

                    MGA_saveJSON('MGA_uiNavigation', UnifiedState.data.uiNavigation);

                    // Rebuild tabs to show/hide favorites bar
                    tabs.innerHTML = buildTabsHTML();
                    setupTabInteractions(tabs);
                });
            });
    ...
    // Build tabs HTML based on legacy mode
    function buildTabsHTML() {
        ...
        if (legacyMode) {
            // Legacy horizontal tabs
            let html = '';
            const allTabs = ['pets', 'abilities', 'seeds', 'values', 'timers', 'rooms', 'hotkeys', 'notifications', 'tools', 'settings', 'help'];
            allTabs.forEach((tabKey, idx) => {
                // Find tab config
                let tabConfig = null;
                Object.keys(TAB_CONFIG).forEach(groupKey => {
                    if (groupKey === 'overview') return;
                    if (TAB_CONFIG[groupKey][tabKey]) {
                        tabConfig = TAB_CONFIG[groupKey][tabKey];
                    }
                });
                if (!tabConfig) return;
                ...
                html += `
                    <div class="mga-tab ${isActive}" data-tab="${tabKey}" data-tooltip="${tabConfig.tooltip}">
                        <span data-icon="${tabConfig.icon}">${tabConfig.icon} ${tabConfig.name}</span>
                        <span class="mga-tab-popout" data-popout="${tabKey}" data-tooltip="Open ${tabConfig.name} in separate window">↗️</span>
                    </div>
                `;
            });
            return html;
        } else {
            // New grouped vertical tabs with overview
            let html = '';

            // Tab search
            html += `<input type="text" class="mga-tab-search" placeholder="Search tabs..." />`;

            // Favorites bar (if any favorites exist)
            if (UnifiedState.data.uiNavigation.favorites.length > 0) {
                html += '<div class="mga-favorites-bar">';
                UnifiedState.data.uiNavigation.favorites.forEach(tabKey => {
                    let tabConfig = null;
                    Object.keys(TAB_CONFIG).forEach(groupKey => {
                        if (groupKey === 'overview') {
                            if (tabKey === 'overview') tabConfig = TAB_CONFIG.overview;
                        } else if (TAB_CONFIG[groupKey][tabKey]) {
                            tabConfig = TAB_CONFIG[groupKey][tabKey];
                        }
                    });
                    if (tabConfig) {
                        const isActive = UnifiedState.activeTab === tabKey ? 'active' : '';
                        html += `
                            <div class="mga-favorite-tab ${isActive}" data-tab="${tabKey}">
                                <span>${tabConfig.icon} ${tabConfig.name}</span>
                            </div>
                        `;
                    }
                });
            ...
            // Overview tab (special - always first, not in a group)
            const overviewActive = UnifiedState.activeTab === 'overview' ? 'active' : '';
            html += `
                <div class="mga-tab-vertical ${overviewActive}" data-tab="overview" data-tooltip="${TAB_CONFIG.overview.tooltip}">
                    <span>${TAB_CONFIG.overview.icon} ${TAB_CONFIG.overview.name}</span>
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <span class="mga-tab-favorite-btn ${UnifiedState.data.uiNavigation.favorites.includes('overview') ? 'favorited' : ''}" data-favorite="overview">★</span>
                        <span class="mga-tab-popout" data-popout="overview" data-tooltip="Open overview in separate window">↗️</span>
                    </div>
                </div>
            `;

            // Tab groups
            ['gameplay', 'analytics', 'config'].forEach(groupKey => {
                const isExpanded = UnifiedState.data.uiNavigation.groupsExpanded[groupKey];
                const collapsedClass = isExpanded ? '' : 'collapsed';

                html += `
                    <div class="mga-tab-group">
                        <div class="mga-tab-group-header ${collapsedClass}" data-group="${groupKey}">
                            <span class="mga-tab-group-icon">▼</span>
                            <span class="mga-tab-group-title">${GROUP_ICONS[groupKey]} ${GROUP_NAMES[groupKey]}</span>
                        </div>
                        <div class="mga-tab-group-tabs">
                `;

                // Add tabs for this group
                Object.keys(TAB_CONFIG[groupKey]).forEach(tabKey => {
                    const tabConfig = TAB_CONFIG[groupKey][tabKey];
                    const isActive = UnifiedState.activeTab === tabKey ? 'active' : '';
                    const isFavorited = UnifiedState.data.uiNavigation.favorites.includes(tabKey);

                    html += `
                        <div class="mga-tab-vertical ${isActive}" data-tab="${tabKey}" data-tooltip="${tabConfig.tooltip}">
                            <span>${tabConfig.icon} ${tabConfig.name}</span>
                            <div style="display: flex; gap: 8px; align-items: center;">
                                <span class="mga-tab-favorite-btn ${isFavorited ? 'favorited' : ''}" data-favorite="${tabKey}">★</span>
                                <span class="mga-tab-popout" data-popout="${tabKey}" data-tooltip="Open ${tabConfig.name} in separate window">↗️</span>
                            </div>
                        </div>
                    `;
    ...
    function createUnifiedUI() {
        ...
        // Tabs - dynamic based on legacy mode
        const tabs = targetDocument.createElement('div');
        const { legacyMode } = UnifiedState.data.uiNavigation;
        tabs.className = legacyMode ? 'mga-tabs mga-scrollable horizontal' : 'mga-tabs';
        tabs.innerHTML = buildTabsHTML();

        // Legacy toggle button
        const legacyToggle = targetDocument.createElement('button');
        legacyToggle.className = 'mga-legacy-toggle';
        legacyToggle.textContent = legacyMode ? 'New UI' : 'Legacy';
        legacyToggle.title = legacyMode ? 'Switch to new grouped UI' : 'Switch to classic horizontal tabs';
        legacyToggle.addEventListener('click', () => {
            UnifiedState.data.uiNavigation.legacyMode = !UnifiedState.data.uiNavigation.legacyMode;
            MGA_saveJSON('MGA_uiNavigation', UnifiedState.data.uiNavigation);
            // Rebuild tabs
            const { legacyMode: newMode } = UnifiedState.data.uiNavigation;
            tabs.className = newMode ? 'mga-tabs mga-scrollable horizontal' : 'mga-tabs';
            tabs.innerHTML = buildTabsHTML();
            legacyToggle.textContent = newMode ? 'New UI' : 'Legacy';
            legacyToggle.title = newMode ? 'Switch to new grouped UI' : 'Switch to classic horizontal tabs';
            setupTabInteractions(tabs);
        });
        ...
        tabsContainer.appendChild(legacyToggle);  // Add legacy toggle
...
<body>
    ...
    function getOverviewTabContent() {
        ...
        const petPresetCount = Object.keys(UnifiedState.data.petPresets).length;
        const roomCount = UnifiedState.data.roomStatus.currentRoom || 'Unknown';

        return `
            <div class="mga-section">
                <div class="mga-section-title">📊 Dashboard Overview</div>

                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px; margin-top: 12px;">
                    <!-- Quick Stats -->
                    <div style="background: rgba(255,255,255,0.05); padding: 12px; border-radius: 6px;">
                        <div style="font-size: 11px; color: #999; margin-bottom: 4px;">Active Pets</div>
                        <div style="font-size: 20px; font-weight: bold; color: #4ade80;">${activePets.length}</div>
                    </div>

                    <div style="background: rgba(255,255,255,0.05); padding: 12px; border-radius: 6px;">
                        <div style="font-size: 11px; color: #999; margin-bottom: 4px;">Pet Presets</div>
                        <div style="font-size: 20px; font-weight: bold; color: #60a5fa;">${petPresetCount}</div>
                    </div>

                    <div style="background: rgba(255,255,255,0.05); padding: 12px; border-radius: 6px;">
                        <div style="font-size: 11px; color: #999; margin-bottom: 4px;">Current Room</div>
                        <div style="font-size: 20px; font-weight: bold; color: #f59e0b;">${roomCount}</div>
                    </div>
                </div>

                <!-- Quick Actions -->
                <div style="margin-top: 16px;">
                    <div style="font-size: 12px; font-weight: bold; margin-bottom: 8px; color: rgba(255,255,255,0.7);">Quick Actions</div>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 8px;">
                        <button class="mga-button" data-quick-action="pets" style="padding: 12px; font-size: 12px;">
                            🐾 Manage Pets
                        </button>
                        <button class="mga-button" data-quick-action="seeds" style="padding: 12px; font-size: 12px;">
                            🌱 Delete Seeds
                        </button>
                        <button class="mga-button" data-quick-action="values" style="padding: 12px; font-size: 12px;">
                            💰 Check Values
                        </button>
                        <button class="mga-button" data-quick-action="rooms" style="padding: 12px; font-size: 12px;">
                            🎮 Room Status
                        </button>
                        <button class="mga-button" data-quick-action="settings" style="padding: 12px; font-size: 12px;">
                            ⚙️ Settings
                        </button>
                    </div>
                </div>
                ...
                <!-- Active Pets Display -->
                <div style="margin-top: 16px;">
                    <div style="font-size: 12px; font-weight: bold; margin-bottom: 8px; color: rgba(255,255,255,0.7);">Currently Active Pets</div>
                    ${activePets.length > 0 ? `
                        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                            ${activePets.map(p => `
                                <div style="background: rgba(255,255,255,0.1); padding: 8px 12px; border-radius: 6px; font-size: 12px;">
                                    <div style="font-weight: bold;">${p.customName || p.species}</div>
                                    <div style="font-size: 10px; color: #999;">Hunger: ${Math.round(p.hunger || 0)}%</div>
                                </div>
                            `).join('')}
                        </div>
                    ` : `
                        <div style="color: #999; font-size: 12px;">No pets currently active</div>
                    `}
                </div>
            </div>
        `;
    ...
    function setupOverviewTabHandlers() {
        // Quick action buttons
        document.querySelectorAll('[data-quick-action]').forEach(btn => {
            btn.addEventListener('click', () => {
                const action = btn.dataset.quickAction;
                // Switch to that tab
                UnifiedState.activeTab = action;

                // Update UI
                const tabs = document.querySelector('.mga-tabs');
                if (tabs) {
                    tabs.querySelectorAll('.mga-tab-vertical, .mga-tab, .mga-favorite-tab').forEach(t => t.classList.remove('active'));
                    const targetTab = tabs.querySelector(`[data-tab="${action}"]`);
    ...
    function loadSavedData() {
        ...
        // Load UI navigation state
        const savedUINavigation = MGA_loadJSON('MGA_uiNavigation', null);
        if (savedUINavigation) {
            UnifiedState.data.uiNavigation = {
                ...UnifiedState.data.uiNavigation,
                ...savedUINavigation
            };
            productionLog('🎨 [UI-NAV] Loaded saved UI navigation state');
        } else {
            productionLog('🎨 [UI-NAV] Using default UI navigation state');
        }

        // Set active tab based on UI mode
        UnifiedState.activeTab = UnifiedState.data.uiNavigation.legacyMode ? 'pets' : 'overview';
        productionLog(`🎨 [UI-NAV] Active tab set to: ${UnifiedState.activeTab} (${UnifiedState.data.uiNavigation.legacyMode ? 'legacy' : 'new'} mode)`);
...
// ==================== HYBRID <-> UNIFIED ADAPTER ====================
// This layer mounts the Unified script's working panel into the Hybrid UI,
// hides the old floating toggle, fixes dragging/sizing, and preserves logic.
// It also adds a dedicated dock item to open the Unified content inside the sidebar.

(function() {
  'use strict';

  const LOG_PREFIX = '[Hybrid-Adapter]';
  const doc = (typeof unsafeWindow !== 'undefined' ? unsafeWindow.document : document);
  const win = (typeof unsafeWindow !== 'undefined' ? unsafeWindow : window);

  // CSS overrides to "change clothes" without altering Unified logic
  const ADAPTER_CSS = `
    /* Hide the old floating round toggle */
    .mga-toggle-btn { display: none !important; }

    /* Make Unified panel act like normal content inside sidebar */
    .mga-panel {
    ...
    .mga-title { color: #e5e7eb !important; }
    .mga-content {
    ...
    /* Tuck the shop overlay under hybrid z-index stack */
    #mga-shop-overlay { z-index: 999997 !important; }

    /* Neutralize dragging classes */
    .mga-dragging, .mga-panel.mga-dragging * { cursor: auto !important; }

    /* Ensure inputs inside Unified don't get hijacked by any global hotkeys */
    .mga-panel input, .mga-panel textarea, .mga-panel select {
      pointer-events: auto !important;
    }

    /* Harmonize fonts with Hybrid */
    .mga-panel, .mga-panel * {
      font-family: Inter, system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, 'Helvetica Neue', Arial, 'Apple Color Emoji','Segoe UI Emoji', 'Segoe UI Symbol' !important;
    }
  `;

  try {
    if (typeof GM_addStyle === 'function') GM_addStyle(ADAPTER_CSS);
    else {
      const style = doc.createElement('style');
      style.textContent = ADAPTER_CSS;
      doc.head.appendChild(style);
    }
  } catch (e) { console.error(LOG_PREFIX, 'Failed to inject adapter CSS', e); }

  // Utility: wait for a selector
  function waitForSelector(sel, { root = doc, timeout = 20000, poll = 100 } = {}) {
    return new Promise((resolve, reject) => {
      const start = Date.now();
      const timer = setInterval(() => {
        const el = root.querySelector(sel);
        if (el) { clearInterval(timer); resolve(el); }
        else if (Date.now() - start > timeout) { clearInterval(timer); reject(new Error('Timeout waiting for ' + sel)); }
      }, poll);
    });
  }

  // Create dock item that opens the Unified content
  function createHybridDockButton() {
    const dock = doc.querySelector('#mgh-dock');
    if (!dock || doc.getElementById('mgh-dock-mgtools')) return;

    const item = doc.createElement('div');
    item.className = 'mgh-dock-item';
    item.id = 'mgh-dock-mgtools';
    item.title = 'MGTools';

    // Icon + tooltip
    item.innerHTML = `<span style="font-size:16px">🌿</span><div class="mgh-tooltip">MGTools</div>`;

    item.addEventListener('click', async () => {
      try {
        const sidebar = doc.querySelector('#mgh-sidebar');
        const titleEl = doc.querySelector('.mgh-sidebar-title');
        const bodyEl = doc.querySelector('.mgh-sidebar-body');
        if (!sidebar || !titleEl || !bodyEl) return;

        // Ensure Unified panel exists and is mounted into the body
        const unifiedPanel = await ensureUnifiedIsMounted(bodyEl);
        titleEl.textContent = 'MGTools';
        sidebar.classList.add('open');

        // Highlight active
        dock.querySelectorAll('.mgh-dock-item').forEach(d => d.classList.remove('active'));
        item.classList.add('active');

        // Scroll content top for UX
        setTimeout(() => { bodyEl.scrollTo({ top: 0, behavior: 'smooth' }); }, 30);
      } catch (err) {
        console.error(LOG_PREFIX, 'click error', err);
      }
    });

    // Insert before any "flip toggle" if present, otherwise append
    const flip = dock.querySelector('.flip-toggle');
    if (flip) dock.insertBefore(item, flip);
    else dock.appendChild(item);
  }

  async function ensureUnifiedIsMounted(sidebarBodyEl) {
    // Wait for the Unified panel to exist
    const unifiedPanel = await waitForSelector('.mga-panel').catch(() => null);
    if (!unifiedPanel) return null;

    // Hide old floating toggle if it exists
    const oldToggle = doc.querySelector('.mga-toggle-btn');
    if (oldToggle) oldToggle.style.display = 'none';

    // Move the Unified panel into the Hybrid sidebar body, but only once
    if (!sidebarBodyEl.contains(unifiedPanel)) {
      // Clear current body + mount
      // BUT: retain anything Hybrid already shows for the rest of tabs - so we wrap Unified in its own container
      let container = doc.getElementById('mgh-unified-container');
      if (!container) {
        container = doc.createElement('div');
        container.id = 'mgh-unified-container';
        container.className = 'mgh-section';
      } else {
        container.innerHTML = '';
      }
      container.appendChild(unifiedPanel);

      // Style tweaks to fit inside sidebar
      unifiedPanel.style.margin = '0';
      unifiedPanel.style.padding = '0';

      // Insert or replace the current content area
      sidebarBodyEl.innerHTML = '';
      sidebarBodyEl.appendChild(container);
    }

    // Safety: disable any drag handlers Unified attaches to its header
    try {
      const header = unifiedPanel.querySelector('.mga-header');
      if (header) {
        header.onmousedown = (e) => { e.stopPropagation(); };
        header.addEventListener('mousedown', (e) => { e.stopPropagation(); }, true);
      }
    } catch (e) {}
  ...
  // Kick off once both UIs are present
  (async function initAdapter() {
    try {
      const [dock] = await Promise.all([
        waitForSelector('#mgh-dock', { timeout: 30000 }),
        waitForSelector('body', { timeout: 30000 })
      ]);

      // Defer a bit to let both scripts finish booting
      setTimeout(() => {
        createHybridDockButton();
      }, 400);
    } catch (e) {
